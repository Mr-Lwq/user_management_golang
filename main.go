...
func main() {
	...
	flag.StringVar(&restPort, "rest-port", "1997", "RESTful server port")
	flag.Parse()

	// 启动 gRPC 服务器
	go func() {
		fmt.Printf("gRPC server started and listening on %service \n", grpcPort)
	...
	// 启动 RESTful 服务器
	go func() {
		fmt.Printf("RESTful server started and listening on %service \n", restPort)
